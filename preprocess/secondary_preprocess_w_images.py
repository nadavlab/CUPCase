...
def get_case_analysis(case_presentation, final_diagnosis, retries=3):
    prompt = (
        f"""Below is a case presentation of a patient, please remove any reference to the final diagnosis {final_diagnosis}
        so it is not present in the resulting text. 
        Do not remove any references to Figures or Images in the text like (Fig 1.)
        Return only the clean text as follows:
        Clean text: <clean text> end.
    ...
    for attempt in range(retries):
        try:
            response = client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "You are an experienced physician. this is important."},
...
def process_case(case_presentation, final_diagnosis):
...
with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
    future_to_case = {
        executor.submit(process_case, row['clean text'], row['final diagnosis']): row['clean text']
